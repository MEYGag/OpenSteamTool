...
- Support auto manifest download via `opensteamtool` / `steamrun` / `wudrm` / `manifestdex` upstream APIs (default is `opensteamtool`), or a custom Lua endpoint (see [Manifest via Lua](#manifest-via-lua)).
...
[manifest]
# Upstream API for depot manifest request codes.  Options: "opensteamtool", "steamrun", "wudrm", "manifestdex"
url = "opensteamtool"
