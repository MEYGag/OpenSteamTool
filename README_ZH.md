...
- 支持通过 `opensteamtool` / `steamrun` / `wudrm` / `manifestdex` 上游 API 自动下载 manifest（默认为 `opensteamtool`），或通过自定义 Lua 端点（参见 [通过 Lua 获取 Manifest](#通过-lua-获取-manifest)）
...
[manifest]
# 仓库 manifest 请求码的上游 API。选项："opensteamtool"、"steamrun"、"wudrm"、"manifestdex"
url = "opensteamtool"
